...
public class SpoolerListener extends ScheduledList<FileEvent> implements ChangeListener<Object> {

    private final Logger LOGGER = LoggerFactory.getLogger(SpoolerListener.class);

    private final FileEventProcessor fileEventProcessor;

    public SpoolerListener(int maxsize, FileEventProcessor fileEventProcessor) {
        super(0, maxsize);
    ...
    public void changed(RunEvent event) {
        if (event.getType() == RunEvent.Type.START) {
            LOGGER.info(event.getChannel().getId() + " - Starting SpoolerListener [" + fileEventProcessor.getClass().getSimpleName() + "]");
            start();
        }
        else if (event.getType() == RunEvent.Type.STOP) {
            LOGGER.info(event.getChannel().getId() + " - Stopping SpoolerListener [" + fileEventProcessor.getClass().getSimpleName() + "]");
            stop();
        }
    }
